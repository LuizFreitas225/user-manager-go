...
func (managerRouter *ManagerRouter) initUserRoutes() {
	...
	userSubRouter.HandleFunc("/findById/:userId", managerRouter.UserController.FindById).Methods(http.MethodGet)
	userSubRouter.HandleFunc("search:searchTerm", managerRouter.UserController.Search).Methods(http.MethodGet)
